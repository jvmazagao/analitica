...
class Extractor:
    ...
    def extract(self, elements):
        single_line = SingleLineParser()
        double_line = DoubleLineParser()
        four_line = FourLineParser()
        six_line = SixLineParser()
        
        if single_line.is_valid(elements):
            return single_line.parse(elements, self.data)

        if double_line.is_valid(elements):
            return double_line.parse(elements, self.data)

        if four_line.is_valid(elements):
            return four_line.parse(elements, self.data)
        
        if six_line.is_valid(elements):
            return six_line.parse(elements, self.data)
