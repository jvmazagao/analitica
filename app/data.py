...
def parse_data(data: dict, elements):
    if len(elements) == 1:
        key = clean_key(elements.pop())
        data[key] = {}
        return data

    if len(elements) == 2:
            key_1, key_2 = [clean_key(x) for x in elements]
    ...
    if len(elements) == 4:
        for i in range(0, 4, 2):
            key, value = clean_key(elements[i]), elements[i+1]
